<template>
  <button class="zh-button" :class="btnClass" :disabled="loading" @click="$emit('click', $event)">
    <zh-icon :icon="icon" v-if="icon && !loading" :size="16" class="icon"></zh-icon>
    <zh-icon icon="zanfill" v-if="loading" :size="16" class="icon"></zh-icon>
    <span class="txt" v-if="$slots.default">
      <slot></slot>
    </span>
  </button>
</template>
    ...
    type: {
      ...
      validator (type) {
        if (type && !['warning', 'success', 'danger', 'info', 'primary'].includes(type)) {
          console.error("type类型必须为： 'warning', 'success', 'danger', 'info', 'primary'")
        }
        return true
      }
    },
    ...
    iconPosition: {
      ...
      validator (type) {
        if (type && !['left', 'right'].includes(type)) {
          console.error("type类型必须为：'left', 'right'")
        }
        return true
      }
    },
